...
(function() {
  var $$, ArgumentDefaultsHelpFormatter, DEBUG, HelpFormatter, HelpFormatter_js, RawDescriptionHelpFormatter, RawTextHelpFormatter, a, adir, ag, ap, args, assert, fmtwindent, formatter, group, group2, parser, parser_a, parser_b, path, pformat, setup, subparsers, util, x, _, _i, _j, _len, _len1, _ref, _ref1, _textwrap,
    __slice = [].slice,
  ...
  exports.HelpFormatter = HelpFormatter = (function() {
    ...
    */
    ...
    HelpFormatter.prototype.add_argument = function(action) {
      ...
      if (action.help !== $$.SUPPRESS) {
        ...
        if (action._get_subactions != null) {
          DEBUG(this._metavar_formatter(action)(1));
    ...
    HelpFormatter.prototype._format_actions_usage = function(actions, groups) {
      ...
      parts = [];
      i = -1;
      for (_l = 0, _len2 = actions.length; _l < _len2; _l++) {
        action = actions[_l];
        i++;
        if (action.help === $$.SUPPRESS) {
          parts.push(null);
    ...
    HelpFormatter.prototype._format_text = function(text) {
      ...
      if (text.match(/%\(prog\)s/) != null) {
        text = text.replace(/%\(prog\)s/, this._prog);
      }
      text_width = this._width - this._current_indent;
  ...
  if (!(module.parent != null)) {
    ...
    formatter.add_usage(parser.usage, parser._actions, parser._mutually_exclusive_groups);
    formatter.add_text(parser.description);
    _ref1 = parser._action_groups;
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      ag = _ref1[_j];
      formatter.start_section(ag.title);
      formatter.add_text(ag.description);
      DEBUG('add group', ag.title, (function() {
        var _k, _len2, _ref2, _results;
        _ref2 = ag._group_actions;
        _results = [];
