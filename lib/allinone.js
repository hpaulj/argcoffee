...
HelpFormatter = (function() {
  ...
  */
  ...
  HelpFormatter.prototype._format_usage = function(usage, actions, groups, prefix) {
    var action_parts, format, get_lines, indent, lines, opt_parts, optionals, parts, pos_parts, positionals, prog, s, text_width;
    if (prefix === null) {
      prefix = 'usage: ';
    }
    if (usage != null) {
      ...
      prog = this._prog;
      optionals = _.filter(actions, function(a) {
        return a.isOptional();
      });
      positionals = _.filter(actions, function(a) {
        return a.isPositional();
      });
      format = this._format_actions_usage;
      action_parts = format([].concat(optionals, positionals), groups);
      assert(_.all((function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = action_parts.length; _i < _len; _i++) {
          s = action_parts[_i];
          _results.push(s.length > 0);
        }
        return _results;
      })()));
      ...
      if (prefix.length + usage.length > text_width) {
        ...
        get_lines = function(parts, indent, prefix) {
          var line, line_len, lines, part, _i, _len;
          if (prefix == null) {
            prefix = null;
          }
          lines = [];
  ...
  HelpFormatter.prototype._format_actions_usage = function(actions, groups) {
    var action, end, group, group_part, part, parts, start, _i, _len;
    parts = [];
    start = 0;
  ...
  HelpFormatter.prototype._format_group_usage = function(group) {
    var action, actions, part, parts, text, _i, _len;
    ...
    if (parts.length > 1) {
      text = parts.join(' | ');
      text = group.required ? "(" + text + ")" : "[" + text + "]";
      parts = [text];
    } else if (parts.length === 0) {
      parts = [];
    } else {
      parts = group.required ? parts : ["[" + parts[0] + "]"];
    }
    return parts;
  };
...
MultiGroupHelpFormatter = (function(_super) {
  ...
  */
  ...
  MultiGroupHelpFormatter.prototype._format_usage = function(usage, actions, groups, prefix) {
    var format, get_lines, indent, lines, opt_parts, optionals, parts, pos_parts, positionals, prog, text_width, _ref4;
    if (prefix === null) {
      prefix = 'usage: ';
    }
    if (usage != null) {
  ...
  MultiGroupHelpFormatter.prototype._format_actions_usage = function(actions, groups) {
    var arg_parts, gactions, group, group_actions, group_parts, optionals, parts, positionals, _i, _len;
  ...
  MultiGroupHelpFormatter.prototype._group_sort = function(actions, groups) {
    var action, group, notInGroups, pgroup, pos_order, positionals, sortGroups, _i, _j, _k, _len, _len1, _len2, _ref4;
    if (groups.length === 0) {
      return groups;
    }
    positionals = _.filter(actions, function(a) {
    ...
    pos_order = function(group) {
      return _.reduce(group._group_actions, (function(prev, a) {
        var x;
        return x = Math.max(prev, positionals.indexOf(a));
      }), -1);
    };
...
ArgumentParser = (function(_super) {
  ...
  ArgumentParser.prototype._parse_known_args = function(arg_strings, namespace) {
    ...
    take_action = function(action, argument_strings, option_string) {
      var actionConflict, argument_values, msg, using_default, _len4, _m, _ref7, _ref8;
      if (option_string == null) {
        option_string = null;
      }
      seen_actions.push(action);
      _ref7 = _this._get_values(action, argument_strings), argument_values = _ref7[0], using_default = _ref7[1];
      if (!using_default) {
        seen_non_default_actions.push(action);
        key = actionHash(action);
        if (actionConflicts[key] != null) {
          _ref8 = actionConflicts[key];
          for (_m = 0, _len4 = _ref8.length; _m < _len4; _m++) {
            actionConflict = _ref8[_m];
  ...
  ArgumentParser.prototype._get_values = function(action, arg_strings) {
    var arg_string, avalue, ii, s, using_default, v, value, _i, _j, _len, _len1, _ref4, _ref5;
    using_default = false;
    if ((_ref4 = action.nargs) !== $$.PARSER && _ref4 !== $$.REMAINDER) {
      switch ('none') {
        case 'all':
