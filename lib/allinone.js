...
var $$, Action, ActionAppend, ActionAppendConstant, ActionCount, ActionHelp, ActionStore, ActionStoreConstant, ActionStoreFalse, ActionStoreTrue, ActionSubparsers, ActionVersion, ArgumentDefaultsHelpFormatter, ArgumentError, ArgumentParser, ArgumentTypeError, DEBUG, FileClass, FileType, HelpFormatter, MultiGroupHelpFormatter, Namespace, RawDescriptionHelpFormatter, RawTextHelpFormatter, TEST, action, assert, fileType, fmtwindent, getattr, hasattr, path, pformat, pnformat, setattr, util, _, _ActionsContainer, _AppendAction, _AppendConstAction, _ArgumentGroup, _CountAction, _HelpAction, _MutuallyExclusiveGroup, _StoreAction, _StoreConstAction, _StoreFalseAction, _StoreTrueAction, _SubParsersAction, _VersionAction, __repr__, _ensure_value, _expand_choices, _format_choices, _get_action_name, _is_mnrep, _ref, _ref1, _ref2, _ref3, _textwrap,
...
_format_choices = function(choices, expand) {
  var choice, choice_strs, rep, result;
  if (expand == null) {
    expand = false;
  }
  rep = ("" + choices).replace(' ', '');
...
_expand_choices = function(choices) {
...
HelpFormatter = (function() {
  ...
  */
  ...
  function HelpFormatter(options) {
    ...
    this._format_just_actions_usage = __bind(this._format_just_actions_usage, this);
    this._format_group_usage = __bind(this._format_group_usage, this);
  ...
  HelpFormatter.prototype._format_usage = function(usage, actions, groups, prefix) {
    var action, action_parts, format, get_lines, indent, lines, opt_parts, optionals, parts, pos_parts, positionals, prog, s, text_width, _i, _len;
    if (prefix === null) {
      prefix = 'usage: ';
    }
    if (usage != null) {
      ...
      if (prefix.length + usage.length > text_width) {
        ...
        get_lines = function(parts, indent, prefix) {
          ...
          lines = [];
          line = [];
          line_len = (prefix != null ? prefix.length : indent.length) - 1;
          for (_j = 0, _len1 = parts.length; _j < _len1; _j++) {
            part = parts[_j];
            if (line.length && line_len + 1 + part.length > text_width) {
  ...
  HelpFormatter.prototype._format_actions_usage = function(actions, groups) {
    var action, end, group, group_part, i, part, parts, start, _i, _len;
    parts = [];
    i = 0;
    while (i < actions.length) {
      ...
      for (_i = 0, _len = groups.length; _i < _len; _i++) {
        group = groups[_i];
        if ((group.no_usage != null) && group.no_usage) {
          continue;
        }
        if (group._group_actions && action === group._group_actions[0]) {
          end = start + group._group_actions.length;
          if (_.isEqual(actions.slice(start, end), group._group_actions)) {
            group_part = this._format_group_usage(group);
      ...
      if (group_part == null) {
        part = this._format_just_actions_usage([action]);
        if (part.length) {
  ...
  HelpFormatter.prototype._format_group_usage = function(group) {
    var action, actions, arg_parts, part, parts, _i, _len;
    actions = group._group_actions;
    parts = [];
    parts.push(group.required ? '(' : '[');
    for (_i = 0, _len = actions.length; _i < _len; _i++) {
      action = actions[_i];
      part = this._format_just_actions_usage([action]);
      if (part.length) {
    ...
    if (parts.length > 1) {
      parts[parts.length - 1] = group.required ? ')' : ']';
    } else {
      parts = [];
    }
    arg_parts = parts.join('');
    arg_parts = arg_parts.replace(/^\(([^|]*)\)$/g, '$1');
    arg_parts = arg_parts.length ? [arg_parts] : [];
    return arg_parts;
  };

  HelpFormatter.prototype._format_just_actions_usage = function(actions) {
    var action, args_string, defaultValue, i, option_string, part, parts, _i, _len;
    parts = [];
  ...
  HelpFormatter.prototype._format_args = function(action, default_metavar) {
    var error, formats, get_metavar, i, nargs, result;
  ...
  HelpFormatter.prototype._expand_help = function(action) {
    var choices_str, name, params, _ref;
...
RawDescriptionHelpFormatter = (function(_super) {
  ...
  function RawDescriptionHelpFormatter() {
    _ref = RawDescriptionHelpFormatter.__super__.constructor.apply(this, arguments);
    return _ref;
  }
...
RawTextHelpFormatter = (function(_super) {
  ...
  function RawTextHelpFormatter() {
    _ref1 = RawTextHelpFormatter.__super__.constructor.apply(this, arguments);
    return _ref1;
  }
...
ArgumentDefaultsHelpFormatter = (function(_super) {
  ...
  function ArgumentDefaultsHelpFormatter() {
    _ref2 = ArgumentDefaultsHelpFormatter.__super__.constructor.apply(this, arguments);
    return _ref2;
  }
  ...
  */
  ...
  ArgumentDefaultsHelpFormatter.prototype._get_help_string = function(action) {
    var defaulting_nargs, help, _ref3;
    help = action.help;
    if (action.help.indexOf('%(defaultValue)s') === -1) {
      if (action.defaultValue !== $$.SUPPRESS) {
        defaulting_nargs = [$$.OPTIONAL, $$.ZERO_OR_MORE];
        if (action.isOptional() || (_ref3 = action.nargs, __indexOf.call(defaulting_nargs, _ref3) >= 0)) {
...
MultiGroupHelpFormatter = (function(_super) {
  __extends(MultiGroupHelpFormatter, _super);

  function MultiGroupHelpFormatter() {
    this._group_sort = __bind(this._group_sort, this);
    this._format_actions_usage = __bind(this._format_actions_usage, this);
    this._format_usage = __bind(this._format_usage, this);
    _ref3 = MultiGroupHelpFormatter.__super__.constructor.apply(this, arguments);
    return _ref3;
  }
  ...
  */
  ...
  MultiGroupHelpFormatter.prototype._format_usage = function(usage, actions, groups, prefix) {
    var action, format, get_lines, indent, lines, opt_parts, optionals, parts, pos_parts, positionals, prog, s, text_width, _i, _len, _ref4;
    if (prefix === null) {
      prefix = 'usage: ';
    }
    if (usage != null) {
      ...
      format = this._format_actions_usage;
      _ref4 = format([].concat(optionals, positionals), groups), opt_parts = _ref4[0], pos_parts = _ref4[1];
      usage = ((function() {
        var _j, _len1, _ref5, _results;
        _ref5 = [].concat([prog], opt_parts, pos_parts);
        _results = [];
  ...
  MultiGroupHelpFormatter.prototype._format_actions_usage = function(actions, groups) {
    var action, arg_parts, gactions, group, group_actions, group_parts, optionals, parts, positionals, _i, _len;
    groups = this._group_sort(actions, groups);
    group_actions = [];
    arg_parts = [];
    for (_i = 0, _len = groups.length; _i < _len; _i++) {
      group = groups[_i];
      gactions = group._group_actions;
      if (!_.isEmpty(_.difference(gactions, actions))) {
        continue;
      }
      group_actions = _.union(group_actions, gactions);
      group_parts = this._format_group_usage(group);
      arg_parts = arg_parts.concat(group_parts);
    }
    actions = _.without.apply(_, [actions].concat(__slice.call(group_actions)));
    optionals = (function() {
      var _j, _len1, _results;
      _results = [];
    ...
    parts = this._format_just_actions_usage(optionals);
    arg_parts = parts.concat(arg_parts);
    parts = this._format_just_actions_usage(positionals);
    return [arg_parts, parts];
  };

  MultiGroupHelpFormatter.prototype._group_sort = function(actions, groups) {
    var action, g, group, i, j, notInGroups, posdex, positionals, samplegroup, sortGroups, _i, _j, _k, _len, _len1, _len2, _ref4, _ref5;
    if (groups.length === 0) {
      return groups;
    }
    positionals = (function() {
    ...
    notInGroups = positionals.slice(0);
    for (i = _i = 0, _len = groups.length; _i < _len; i = ++_i) {
      ...
      for (_j = 0, _len1 = _ref4.length; _j < _len1; _j++) {
        ...
        if (j > -1) {
          [].splice.apply(notInGroups, [j, j - j + 1].concat(_ref5 = [])), _ref5;
        }
      }
    }
    sortGroups = groups.slice(0);
    samplegroup = group;
    for (_k = 0, _len2 = notInGroups.length; _k < _len2; _k++) {
      action = notInGroups[_k];
    ...
    sortGroups = _.sortBy(sortGroups, function(a) {
      return a[1];
    });
    sortGroups = (function() {
      var _l, _len3, _results;
      _results = [];
      for (_l = 0, _len3 = sortGroups.length; _l < _len3; _l++) {
        i = sortGroups[_l];
        _results.push(i[0]);
      }
      return _results;
  ...
  return MultiGroupHelpFormatter;
...
_get_action_name = function(argument) {
  ...
  } else if ((_ref4 = argument.metavar) !== null && _ref4 !== $$.SUPPRESS) {
    return argument.metavar;
  } else if ((_ref5 = argument.dest) !== null && _ref5 !== $$.SUPPRESS) {
...
Action = (function() {
  ...
  */
  ...
  Action.prototype.getName = function() {
    ...
    } else if ((_ref4 = this.metavar) != null ? _ref4 : this.metavar !== $$.SUPPRESS) {
      return this.metavar;
    } else if ((_ref5 = this.dest) != null ? _ref5 : this.dest !== $$.SUPPRESS) {
      return this.dest;
    }
  };
...
_SubParsersAction = (function(_super) {
  ...
  function _SubParsersAction(options) {
    this._getSubactions = __bind(this._getSubactions, this);
    this._get_subactions = __bind(this._get_subactions, this);
    var _ref4, _ref5, _ref6;
    this._prog_prefix = options.prog;
    this._parser_class = (_ref4 = options.parser_class) != null ? _ref4 : options.parserClass;
    this._name_parser_map = {};
    this._choices_actions = [];
    options.dest = (_ref5 = options.dest) != null ? _ref5 : $$.SUPPRESS;
    options.nargs = $$.PARSER;
    options.choices = this._name_parser_map;
    options.required = (_ref6 = options.required) != null ? _ref6 : true;
  ...
  _SubParsersAction.prototype.add_parser = function(name, options) {
    var alias, aliases, choice_action, help, parser, _i, _len;
    if (options == null) {
      options = {};
    }
  ...
  _SubParsersAction.prototype.__call__ = function(parser, namespace, values, option_string) {
    var arg_strings, astring, choices, msg, parser_name, _i, _len, _ref4, _ref5, _results;
    if (option_string == null) {
      option_string = null;
    }
    ...
    parser = (_ref4 = this._name_parser_map[parser_name]) != null ? _ref4 : null;
    if (parser === null) {
      choices = _.keys(this._name_parser_map).join(', ');
      msg = "unknown parser " + parser_name + " (choices: " + choices + ")";
      throw new ArgumentError(this, msg);
    }
    _ref5 = parser.parse_known_args(arg_strings, namespace), namespace = _ref5[0], arg_strings = _ref5[1];
...
FileClass = (function() {
  ...
  */
  ...
  FileClass.prototype.call = function(filename) {
    var createStream, error, fd, flags, msg, stream;
...
fileType = function(options) {
  var createStream, flags, fn, fs, msg, std, _ref4, _ref5;
  if (options == null) {
    options = {
      flags: 'r'
    };
  ...
  if (flags === 'r') {
    _ref4 = [process.stdin, fs.createReadStream], std = _ref4[0], createStream = _ref4[1];
  } else if (flags === 'w') {
    _ref5 = [process.stdout, fs.createWriteStream], std = _ref5[0], createStream = _ref5[1];
  } else {
...
_ActionsContainer = (function() {
  function _ActionsContainer(options) {
    ...
    this._handle_conflict_resolve = __bind(this._handle_conflict_resolve, this);
    this._add_container_actions = __bind(this._add_container_actions, this);
    this.description = options.description;
    this.argument_default = options.argument_default;
    this.prefix_chars = (_ref4 = options.prefixChars) != null ? _ref4 : options.prefix_chars;
  ...
  _ActionsContainer.prototype._registry_get = function(registry_name, value, defaultValue) {
    ...
    return (_ref4 = this._registries[registry_name][value]) != null ? _ref4 : defaultValue;
  };
  ...
  _ActionsContainer.prototype.get_default = function(dest) {
    ...
    return (_ref5 = this._defaults[dest]) != null ? _ref5 : null;
  };
  ...
  _ActionsContainer.prototype.add_argument = function() {
    var action_class, args, chars, dest, options, type_func, _i, _ref4;
    ...
    if (args.length === 0 || (args.length === 1 && (_ref4 = args[0][0], __indexOf.call(chars, _ref4) < 0))) {
  ...
  _ActionsContainer.prototype.add_mutually_exclusive_group = function(options, args) {
    var group, _i, _len;
    if (options == null) {
      options = {};
    }
  ...
  _ActionsContainer.prototype._add_action = function(action) {
    var option_string, _i, _j, _len, _len1, _ref4, _ref5;
    this._check_conflict(action);
    this._actions.push(action);
    action.container = this;
    _ref4 = action.option_strings;
    for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
      option_string = _ref4[_i];
      this._option_string_actions[option_string] = action;
    }
    _ref5 = action.option_strings;
    for (_j = 0, _len1 = _ref5.length; _j < _len1; _j++) {
      option_string = _ref5[_j];
  ...
  _ActionsContainer.prototype._add_container_actions = function(container) {
    var actionHash, ctr, group, group_map, msg, mutex_group, title_group_map, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref10, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9, _results;
    title_group_map = {};
    _ref4 = this._action_groups;
    for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
      group = _ref4[_i];
    ...
    _ref5 = container._action_groups;
    for (_j = 0, _len1 = _ref5.length; _j < _len1; _j++) {
      group = _ref5[_j];
      if (!(group.title in title_group_map)) {
  ...
  _ActionsContainer.prototype._get_positional_options = function(dest, options) {
    ...
    */
    ...
    if ((_ref4 = options.nargs) !== $$.OPTIONAL && _ref4 !== $$.ZERO_OR_MORE) {
      options.required = true;
  ...
  _ActionsContainer.prototype._check_conflict = function(action) {
    var confl_optional, confl_optionals, conflict_handler, option_string, _i, _len, _ref4;
    confl_optionals = [];
    _ref4 = action.option_strings;
    for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
      option_string = _ref4[_i];
  ...
  _ActionsContainer.prototype._handle_conflict_resolve = function(action, conflicting_actions) {
    var i, option_string, _i, _len, _ref4, _results;
    _results = [];
    for (_i = 0, _len = conflicting_actions.length; _i < _len; _i++) {
      _ref4 = conflicting_actions[_i], option_string = _ref4[0], action = _ref4[1];
...
_ArgumentGroup = (function(_super) {
  ...
  function _ArgumentGroup(container, options) {
    ...
    options.prefix_chars = (_ref4 = options.prefixChars) != null ? _ref4 : container.prefix_chars;
    options.argument_default = (_ref5 = options.argument_default) != null ? _ref5 : container.argument_default;
    options.conflict_handler = (_ref6 = options.conflict_handler) != null ? _ref6 : container.conflict_handler;
...
Namespace = (function() {
  ...
  Namespace.prototype.get = function(key, defaultValue) {
    var _ref4;
    return (_ref4 = this[key]) != null ? _ref4 : defaultValue;
  };
...
ArgumentParser = (function(_super) {
  ...
  function ArgumentParser(options) {
    var acoptions, defaultKey, default_prefix, parent, _i, _len, _ref10, _ref11, _ref12, _ref13, _ref14, _ref15, _ref16, _ref17, _ref18, _ref19, _ref20, _ref21, _ref22, _ref23, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    if (options == null) {
      options = {};
    }
    ...
    this._check_argument = __bind(this._check_argument, this);
    this.prog = (_ref4 = options.prog) != null ? _ref4 : path.basename(process.argv[1]);
    this.usage = (_ref5 = options.usage) != null ? _ref5 : null;
    this.epilog = (_ref6 = options.epilog) != null ? _ref6 : null;
    this.parents = (_ref7 = options.parents) != null ? _ref7 : [];
    this.formatter_class = (_ref8 = (_ref9 = options.formatter_class) != null ? _ref9 : options.formatterClass) != null ? _ref8 : HelpFormatter;
    this.fromfile_prefix_chars = (_ref10 = (_ref11 = options.fromfile_prefix_chars) != null ? _ref11 : options.fromfilePrefixChars) != null ? _ref10 : null;
    this.add_help = (_ref12 = (_ref13 = options.addHelp) != null ? _ref13 : options.add_help) != null ? _ref12 : true;
    this.debug = (_ref14 = options.debug) != null ? _ref14 : false;
    this.description = (_ref15 = options.description) != null ? _ref15 : null;
    this.prefix_chars = (_ref16 = (_ref17 = options.prefixChars) != null ? _ref17 : options.prefix_chars) != null ? _ref16 : '-';
    this.argument_default = (_ref18 = (_ref19 = options.argumentDefault) != null ? _ref19 : options.argument_default) != null ? _ref18 : null;
    this.conflict_handler = (_ref20 = (_ref21 = options.conflict_handler) != null ? _ref21 : options.conflictHandler) != null ? _ref20 : 'error';
    this.args_default_to_positional = (_ref22 = options.args_default_to_positional) != null ? _ref22 : false;
    acoptions = {
  ...
  ArgumentParser.prototype.add_subparsers = function(options) {
    var ParsersClass, description, formatter, groups, positionals, title, _ref4, _ref5, _ref6;
    if (options == null) {
      options = {};
    }
    ...
    if ((options.title != null) || (options.description != null)) {
      title = (_ref4 = options.title) != null ? _ref4 : 'subcommands';
      description = (_ref5 = options.description) != null ? _ref5 : null;
    ...
    if (options.prog == null) {
      ...
      groups = (_ref6 = this._mutuallyExclusiveGroups) != null ? _ref6 : this._mutually_exclusive_groups;
  ...
  ArgumentParser.prototype.parse_args = function(args, namespace) {
    ...
    _ref4 = this.parse_known_args(args, namespace), args = _ref4[0], argv = _ref4[1];
    if (argv.length > 0) {
  ...
  ArgumentParser.prototype.parse_known_args = function(args, namespace) {
    var argv, dest, error, _default, _i, _len, _ref4, _ref5;
    if (args == null) {
      args = null;
    }
    if (namespace == null) {
    ...
    try {
      _ref5 = this._parse_known_args(args, namespace), namespace = _ref5[0], args = _ref5[1];
  ...
  ArgumentParser.prototype._parse_known_args = function(arg_strings, namespace) {
    var actionConflicts, actionHash, action_used, arg_string, arg_string_pattern, arg_string_pattern_parts, conflicts, consume_loop, consume_optional, consume_positionals, extras, gactions, group, group_actions, i, ii, k, key, msg, mutex_action, mutex_group, mxgroups, names, opt_actions, option_string_indices, option_tuple, pattern, penult, positionals, required_actions, seen_actions, seen_non_default_actions, take_action, v, x, _cnt, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _p, _ref10, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
      _this = this;
    ...
    mxgroups = (_ref4 = this._mutuallyExclusiveGroups) != null ? _ref4 : this._mutually_exclusive_groups;
    for (_i = 0, _len = mxgroups.length; _i < _len; _i++) {
      mutex_group = mxgroups[_i];
      group_actions = (_ref5 = mutex_group._groupActions) != null ? _ref5 : mutex_group._group_actions;
    ...
    take_action = function(action, argument_strings, option_string) {
      var actionConflict, argument_values, msg, _len4, _m, _ref7;
      if (option_string == null) {
        option_string = null;
      }
      ...
      if (argument_values !== action.defaultValue) {
        ...
        if (actionConflicts[key] != null) {
          _ref7 = actionConflicts[key];
          for (_m = 0, _len4 = _ref7.length; _m < _len4; _m++) {
            actionConflict = _ref7[_m];
    ...
    consume_optional = function(start_index, no_action, penult) {
      var action_tuples, arg_count, args, chars, explicit_arg, match_argument, msg, new_explicit_arg, option_string, optionals_map, selected_patterns, shared_count, slots, start, stop, tuple, _len4, _m, _ref7;
      if (no_action == null) {
        no_action = false;
      }
      if (penult == null) {
      ...
      while (true) {
        ...
        if (explicit_arg != null) {
          ...
          if (arg_count === 0 && (_ref7 = option_string[1], __indexOf.call(chars, _ref7) < 0)) {
          ...
          if (_this._is_nargs_variable(action)) {
            slots = _this._match_arguments_partial([action].concat(positionals), selected_patterns);
            DEBUG('    opt+pos slots', slots);
            shared_count = slots[0];
          } else {
            shared_count = null;
          }
          if ((shared_count != null) && _.str.count(selected_patterns, 'O') <= penult) {
    ...
    positionals = this._get_positional_actions();
    consume_positionals = function(start_index, no_action) {
      var arg_count, arg_counts, args, ii, match_partial, pats, selected_pattern, _len4, _m, _ref10, _ref11, _ref7, _ref8, _ref9;
      if (no_action == null) {
        no_action = false;
      }
      ...
      _ref7 = _.zipShortest(positionals, arg_counts);
      for (_m = 0, _len4 = _ref7.length; _m < _len4; _m++) {
        _ref8 = _ref7[_m], action = _ref8[0], arg_count = _ref8[1];
        args = arg_strings.slice(start_index, start_index + arg_count);
        if ((_ref9 = action.nargs) !== $$.PARSER && _ref9 !== $$.REMAINDER) {
          pats = arg_string_pattern.slice(start_index, start_index + arg_count);
          DEBUG('take action:', action.dest, args, pats);
          ii = pats.indexOf('-');
          if (ii > -1) {
            assert(args[ii] === '--');
            [].splice.apply(args, [ii, ii - ii + 1].concat(_ref10 = [])), _ref10;
      ...
      [].splice.apply(positionals, [0, 9e9].concat(_ref11 = positionals.slice(arg_counts.length))), _ref11;
      return start_index;
    };
    consume_loop = function(no_action, penult) {
      var foo, index_keys, max_option_string_index, next_option_string_index, positionals_end_index, start_index, stop_index, strings;
      if (no_action == null) {
        no_action = false;
      }
      if (penult == null) {
      ...
      index_keys = (function() {
        ...
        _ref7 = _.keys(option_string_indices);
        _results = [];
        for (_m = 0, _len4 = _ref7.length; _m < _len4; _m++) {
          x = _ref7[_m];
          _results.push(+x);
    ...
    penult = _.str.count(arg_string_pattern, 'O');
    opt_actions = [
      (function() {
        var _results;
        _results = [];
        for (k in option_string_indices) {
          v = option_string_indices[k];
          if (v[0]) {
            _results.push(v[0]);
          }
        }
    ...
    _cnt = 0;
    if (this._is_nargs_variable(opt_actions) && positionals && penult > 1) {
      for (ii = _m = 0; 0 <= penult ? _m < penult : _m > penult; ii = 0 <= penult ? ++_m : --_m) {
        extras = [];
        positionals = this._get_positional_actions();
    ...
    action_used = false;
    _ref9 = (_ref8 = this._mutuallyExclusiveGroups) != null ? _ref8 : this._mutually_exclusive_groups;
    for (_o = 0, _len5 = _ref9.length; _o < _len5; _o++) {
      group = _ref9[_o];
      if (group.required) {
        gactions = (_ref10 = group._groupActions) != null ? _ref10 : group._group_actions;
        for (_p = 0, _len6 = gactions.length; _p < _len6; _p++) {
          action = gactions[_p];
        ...
        if (!action_used) {
          names = (function() {
            var _len7, _q, _results;
            _results = [];
            for (_q = 0, _len7 = gactions.length; _q < _len7; _q++) {
              action = gactions[_q];
              if (action.help !== $$.SUPPRESS) {
  ...
  ArgumentParser.prototype._read_args_from_files = function(arg_strings) {
    var arg, arg_line, arg_string, argstrs, content, error, filename, firstchar, fs, new_arg_strings, _i, _j, _k, _len, _len1, _len2, _ref4;
    fs = require('fs');
    new_arg_strings = [];
    for (_i = 0, _len = arg_strings.length; _i < _len; _i++) {
      ...
      if (__indexOf.call(this.fromfile_prefix_chars, firstchar) < 0) {
        ...
        try {
          ...
          for (_j = 0, _len1 = content.length; _j < _len1; _j++) {
            arg_line = content[_j];
            _ref4 = this.convert_arg_line_to_args(arg_line);
  ...
  ArgumentParser.prototype._read_args_from_files1 = function(arg_strings) {
    ...
    */
    ...
    arg_strings.forEach(function(arg_string) {
      ...
      var argstrs, content, error, filename;
  ...
  ArgumentParser.prototype._match_argument = function(action, arg_strings_pattern) {
    var args_errors, matches, msg, nargs_pattern, _ref4;
    ...
    if (matches == null) {
      ...
      msg = (_ref4 = args_errors[action.nargs]) != null ? _ref4 : "expected " + action.nargs + " argument(s)";
      throw new ArgumentError(action, msg);
    }
  ...
  ArgumentParser.prototype._match_arguments_partial = function(actions, arg_strings_pattern) {
    var actions_slice, foo, i, m, pattern, result, strlength, _i, _ref4;
    result = [];
    foo = this._get_nargs_pattern;
    strlength = function(string) {
      return string.length;
    };
    for (i = _i = _ref4 = actions.length; _ref4 <= 0 ? _i <= 0 : _i >= 0; i = _ref4 <= 0 ? ++_i : --_i) {
  ...
  ArgumentParser.prototype._parse_optional = function(arg_string) {
    var actions, explicit_arg, option_string, option_tuple, option_tuples, options, tup, _ref4, _ref5;
    assert(this.prefix_chars != null);
    if (!arg_string) {
      return null;
    }
    if (!(_ref4 = arg_string[0], __indexOf.call(this.prefix_chars, _ref4) >= 0)) {
      return null;
    }
    ...
    if (__indexOf.call(arg_string, '=') >= 0) {
      _ref5 = arg_string.split('='), option_string = _ref5[0], explicit_arg = _ref5[1];
    ...
    if (option_tuples.length > 1) {
      options = (function() {
        ...
        for (_i = 0, _len = option_tuples.length; _i < _len; _i++) {
          _ref6 = option_tuples[_i], action = _ref6[0], option_string = _ref6[1], explicit_arg = _ref6[2];
  ...
  ArgumentParser.prototype._get_option_tuples = function(option_string) {
    var actions, chars, explicit_arg, option_prefix, result, short_explicit_arg, short_option_prefix, tup, _ref4, _ref5, _ref6, _ref7, _ref8;
    result = [];
    chars = this.prefix_chars;
    if ((_ref4 = option_string[0], __indexOf.call(chars, _ref4) >= 0) && (_ref5 = option_string[1], __indexOf.call(chars, _ref5) >= 0)) {
      if (__indexOf.call(option_string, '=') >= 0) {
        _ref6 = option_string.split('='), option_prefix = _ref6[0], explicit_arg = _ref6[1];
      } else {
    ...
    } else if ((_ref7 = option_string[0], __indexOf.call(chars, _ref7) >= 0) && (_ref8 = option_string[1], __indexOf.call(chars, _ref8) < 0)) {
      option_prefix = option_string;
      explicit_arg = null;
  ...
  ArgumentParser.prototype._is_nargs_variable = function(action) {
    var a, _ref4;
    if (_.isArray(action)) {
      return _.any((function() {
        ...
        for (_i = 0, _len = action.length; _i < _len; _i++) {
          a = action[_i];
          _results.push(this._is_nargs_variable(a));
        }
        return _results;
      }).call(this));
    } else {
      if ((_ref4 = action.nargs) === $$.OPTIONAL || _ref4 === $$.ZERO_OR_MORE || _ref4 === $$.ONE_OR_MORE || _ref4 === $$.REMAINDER || _ref4 === $$.PARSER) {
        return true;
      }
  ...
  ArgumentParser.prototype.parse_intermixed_args = function(args, namespace) {
    var argv, msg, _ref4;
    if (args == null) {
      args = null;
    }
    ...
    _ref4 = this.parse_known_intermixed_args(args, namespace), args = _ref4[0], argv = _ref4[1];
    if (argv.length > 0) {
      msg = "unrecognized arguments: " + (argv.join(' '));
      this.error(msg);
    }
  ...
  ArgumentParser.prototype.parse_known_intermixed_args = function(args, namespace) {
    var a, extras, group, optionals, positionals, remaining_args, save_usage, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref4, _ref5, _ref6;
    if (args == null) {
      args = null;
    }
    if (namespace == null) {
    ...
    a = (function() {
      ...
      for (_i = 0, _len = positionals.length; _i < _len; _i++) {
        ...
        if ((_ref4 = action.nargs) === $$.PARSER || _ref4 === $$.REMAINDER) {
          _results.push(action);
        }
      }
      return _results;
    ...
    try {
        ...
        for (_j = 0, _len1 = positionals.length; _j < _len1; _j++) {
          ...
          if (action.nargs === 0) {
            ...
            if (namespace[action.dest] != null) {
              this.error("did not expect a suppressed action value");
            }
          }
        }
      } finally {
      ...
      try {
        _ref5 = this.parse_known_args(remaining_args, namespace), namespace = _ref5[0], extras = _ref5[1];
      } finally {
        for (_n = 0, _len5 = optionals.length; _n < _len5; _n++) {
          action = optionals[_n];
          action.required = action.save_required;
        }
        _ref6 = this._mutually_exclusive_groups;
  ...
  ArgumentParser.prototype._get_values = function(action, arg_strings) {
    var arg_string, avalue, ii, s, v, value, _i, _j, _len, _len1, _ref4, _ref5;
    if ((_ref4 = action.nargs) !== $$.PARSER && _ref4 !== $$.REMAINDER) {
      switch ('none') {
        case 'all':
          arg_strings = (function() {
    ...
    if (arg_strings.length === 0 && action.nargs === $$.OPTIONAL) {
      ...
      if (_.isArray(value)) {
        for (_i = 0, _len = value.length; _i < _len; _i++) {
          avalue = value[_i];
          this._check_value(action, avalue);
        }
      } else {
        this._check_value(action, value);
      }
    } else if (arg_strings.length === 1 && ((_ref5 = action.nargs) === null || _ref5 === $$.OPTIONAL)) {
      ...
      value = (function() {
        var _j, _len1, _results;
        _results = [];
        for (_j = 0, _len1 = arg_strings.length; _j < _len1; _j++) {
          v = arg_strings[_j];
  ...
  ArgumentParser.prototype._get_value = function(action, arg_string) {
    var error, msg, name, result, type_func;
  ...
  ArgumentParser.prototype.format_help = function() {
    var actionGroup, formatter, _i, _len, _ref4, _ref5, _ref6;
    formatter = this._get_formatter();
    formatter.addUsage(this.usage, this._actions, this._mutually_exclusive_groups);
    formatter.addText(this.description);
    _ref5 = (_ref4 = this._actionGroups) != null ? _ref4 : this._action_groups;
    for (_i = 0, _len = _ref5.length; _i < _len; _i++) {
      actionGroup = _ref5[_i];
      formatter.startSection(actionGroup.title);
      formatter.addText(actionGroup.description);
      formatter.addArguments((_ref6 = actionGroup._groupActions) != null ? _ref6 : actionGroup._group_actions);
  ...
  ArgumentParser.prototype.parseIntermixedArgs = ArgumentParser.prototype.parse_intermixed_args;

  ArgumentParser.prototype.parseKnownIntermixedArgs = ArgumentParser.prototype.parse_known_intermixed_args;
...
if (TEST) {
  (function() {
    var Nsp, args, c1, c2, childParser, error, parentParser, parser, parser2, subparsers, testparse;
    ...
    if (1) {
      ...
      if (1) {
        ...
        console.log('child optional actions: ', (function() {
          var _i, _len, _ref4, _results;
          _ref4 = childParser._optionals._group_actions;
          _results = [];
          for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
            action = _ref4[_i];
        ...
        console.log('child positional actions: ', (function() {
          var _i, _len, _ref4, _results;
          _ref4 = childParser._positionals._group_actions;
          _results = [];
          for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
            action = _ref4[_i];
            _results.push(action.dest);
