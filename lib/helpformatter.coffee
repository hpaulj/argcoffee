...
class Section
    constructor: (@_parent, @_heading, @_items=[]) ->
    ...
    formatHelp: (formatter) ->
        ...
        heading = ''
        if @_heading? && @_heading != $$.SUPPRESS
            currentIndent = formatter.currentIndent
...
HelpFormatter.prototype._splitLines = function (text, width) {
  ...
  return lines;
    ...
    _fillText: (text, width, indent) ->
        lines = @_splitLines(text, width)
        lines.foreach( (line) -> line = indent + line)
        lines.join($$.EOL)

    _getHelpString: (action) ->
        action.help

exports.HelpFormater = HelpFormatter
