...
HelpFormatter.prototype._formatActionsUsage = function (actions, groups) {
  ...
  groups.forEach(function (group) {
    var end;
    var i;
group._groupActions = group._group_actions;
  ...
  text = text.replace(/([\[(]) /,'$1'); // remove spaces
  text = text.replace(/ ([\])])/,'$1');
  text = text.replace(/\[ *\]/, ''); // remove empty groups
  text = text.replace(/\( *\)/, '');
  text = text.replace(/\(([^|]*)\)/, '$1'); // remove () from single action groups
