// Generated by CoffeeScript 1.4.0
(function() {
  var Annotation, ArgumentParser, DEBUG, PARSER_CFG, alt_getarglist, annotation_from, annotations, argparse, assert, call, formal_parameter_list, getargspec, getfullargspec, is_annotation, iterable, main, parser_from, path, pconf, util, _, _extract_kwargs, _match_cmd, _parser_registry,
    __slice = [].slice,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  if (true) {
    DEBUG = function() {
      var arg;
      arg = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      arg.unshift('==> ');
      return console.log.apply(console, arg);
    };
  } else {
    DEBUG = function() {};
  }

  util = require('util');

  assert = require('assert');

  path = require('path');

  _ = require('underscore');

  _.str = require('underscore.string');

  argparse = require('argcoffee');

  // http://stackoverflow.com/questions/6921588/is-it-possible-to-reflect-the-arguments-of-a-javascript-function
var FN_ARGS = /^function\s*[^\(]*\(\s*([^\)]*)\)/m;
var FN_ARG_SPLIT = /,/;
var FN_ARG = /^\s*(_?)(\S+?)\1\s*$/;
var STRIP_COMMENTS = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg;

function formalParameterList(fn) {
   var fnText,argDecl;
   var args=[];
   fnText = fn.toString().replace(STRIP_COMMENTS, '');
   argDecl = fnText.match(FN_ARGS); 

   var r = argDecl[1].split(FN_ARG_SPLIT);
   for(var a in r){
      var arg = r[a];
      arg.replace(FN_ARG, function(all, underscore, name){
         args.push(name);
      });
   }
   return args;
 };


  formal_parameter_list = function(fn) {
    var FN_ARG, FN_ARGS, FN_ARG_SPLIT, STRIP_COMMENTS, a, arg, arg_decl, args, fn_text, r;
    FN_ARGS = /^function\s*[^\(]*\(\s*([^\)]*)\)/m;
    FN_ARG_SPLIT = /,/;
    FN_ARG = /^\s*(_?)(\S+?)\1\s*$/;
    STRIP_COMMENTS = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg;
    args = [];
    fn_text = fn.toString().replace(STRIP_COMMENTS, '');
    arg_decl = fn_text.match(FN_ARGS);
    r = arg_decl[1].split(FN_ARG_SPLIT);
    for (a in r) {
      arg = r[a];
      arg.replace(FN_ARG, function(all, underscore, name) {
        return args.push(name);
      });
    }
    return args;
  };

  alt_getarglist = function(fn) {
    return fn.toString().match(/function\s+\w*\s*\((.*?)\)/)[1].split(/\s*,\s*/);
  };

  getfullargspec = (function() {

    function getfullargspec(f) {
      var _ref;
      this.args = alt_getarglist(f);
      this.varargs = [];
      this.varkw = {};
      this.defaults = [];
      this.annotations = (_ref = f.__annotations__) != null ? _ref : {};
    }

    return getfullargspec;

  })();

  getargspec = function(callableobj) {
    var argspec, name, _ref;
    if (_.isFunction(callableobj)) {
      argspec = new getfullargspec(callableobj);
      name = (_ref = callableobj.name) != null ? _ref : '';
    } else {
      throw new TypeError('Could not determine the signature of' + callableobj);
    }
    DEBUG(name, argspec);
    return argspec;
  };

  annotations = function(ann) {
    var annotate;
    annotate = function(f) {
      var argname, args, fas, _ref;
      fas = getargspec(f);
      args = fas.args;
      for (argname in ann) {
        if (_ref = !argname, __indexOf.call(args, _ref) >= 0) {
          thrown(new Error('Annotating non-existing argument' + argname));
        }
      }
      f.__annotations__ = ann;
      return f;
    };
    return annotate;
  };

  exports.annotations = annotations;

  is_annotation = function(obj) {
    return (obj.help != null) && (obj.kind != null) && (obj.abbrev != null) && (obj.type != null) && (obj.choices != null) && (obj.metaver != null);
  };

  Annotation = (function() {

    function Annotation(help, kind, abbrev, type, choices, metavar) {
      var _ref;
      this.help = help != null ? help : null;
      this.kind = kind != null ? kind : 'positional';
      this.abbrev = abbrev != null ? abbrev : null;
      this.type = type != null ? type : null;
      this.choices = choices != null ? choices : null;
      this.metavar = metavar != null ? metavar : null;
      assert((_ref = this.kind) === 'positional' || _ref === 'option' || _ref === 'flag');
      if (this.kind === 'positional') {
        assert(this.abbrev === null);
      }
    }

    return Annotation;

  })();

  annotation_from = function(obj) {
    if (is_annotation(obj)) {
      return obj;
    } else if (_.isArray(obj)) {
      return (function(func, args, ctor) {
        ctor.prototype = func.prototype;
        var child = new ctor, result = func.apply(child, args);
        return Object(result) === result ? result : child;
      })(Annotation, obj, function(){});
    }
    return new Annotation(obj);
  };

  /*
  in js version, ArgumentParser takes one argument 'options'
  possible attributes of options are:
  prog, usage, epilog, parents, formatter_class, fromfile_prefix_chars, 
  add_help, debug, description, prefix_chars, argument_default, conflict_handler,
  version
  */


  PARSER_CFG = 'prog, usage, epilog, parents, formatter_class, fromfile_prefix_chars, ' + 'add_help, debug, description, prefix_chars, argument_default, conflict_handler, ' + 'version';

  PARSER_CFG = PARSER_CFG.split(', ');

  DEBUG(PARSER_CFG);

  pconf = function(obj) {
    var cfg, name;
    cfg = {
      description: 'obj doc',
      formatter_class: argparse.HelpFormatter
    };
    for (name in obj) {
      if (__indexOf.call(PARSER_CFG, name) >= 0) {
        cfg[name] = obj[name];
      }
    }
    return cfg;
  };

  _parser_registry = {};

  parser_from = function(obj, confparams) {
    var conf, parser, _ref, _ref1;
    if (confparams == null) {
      confparams = {};
    }
    if (_parser_registry[obj] != null) {
      return _parser_registry[obj];
    }
    conf = _.extend({}, pconf(obj), confparams);
    _parser_registry[obj] = parser = new ArgumentParser(conf);
    parser.obj = obj;
    parser.case_sensitive = (_ref = (_ref1 = confparams['case_sensitive']) != null ? _ref1 : obj['case_sensitive']) != null ? _ref : true;
    if ((obj['commands'] != null) && !_.isClass(obj)) {
      parser.addsubcommands(obj.commands, obj, 'subcommands');
    } else {
      parser.populate_from(obj);
    }
    return parser;
  };

  exports.parser_from = parser_from;

  _extract_kwargs = function(args) {
    var arg, arglist, kwargs, m, name, _i, _len;
    arglist = [];
    kwargs = {};
    for (_i = 0, _len = args.length; _i < _len; _i++) {
      arg = args[_i];
      m = arg.match(/([a-zA-Z_]\w*])=/);
      if (m != null) {
        name = m[1];
        kwargs[name] = arg.slice(name.length + 1);
      } else {
        arglist.push(arg);
      }
    }
    return [arglist, kwargs];
  };

  _match_cmd = function(abbrev, commands, case_sensitive) {
    var c, matches, n, name, perfect_matches;
    if (case_sensitive == null) {
      case_sensitive = true;
    }
    if (!case_sensitive) {
      abbrev = abbrev.toUpper();
      commands = (function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = commands.length; _i < _len; _i++) {
          c = commands[_i];
          _results.push(c.toUpper());
        }
        return _results;
      })();
    }
    perfect_matches = (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = commands.length; _i < _len; _i++) {
        name = commands[_i];
        if (name === abbrev) {
          _results.push(name);
        }
      }
      return _results;
    })();
    if (perfect_matches.length === 1) {
      return perfect_matches[0];
    }
    matches = (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = commands.length; _i < _len; _i++) {
        name = commands[_i];
        if (_.str.startsWith(name, abbrev)) {
          _results.push(name);
        }
      }
      return _results;
    })();
    n = matches.length;
    if (n === 1) {
      return matches[0];
    } else if (n > 1) {
      throw Error("Ambiguous command " + abbrev + " matching " + matches);
    }
  };

  ArgumentParser = (function(_super) {
    var case_sensitive;

    __extends(ArgumentParser, _super);

    function ArgumentParser() {
      return ArgumentParser.__super__.constructor.apply(this, arguments);
    }

    case_sensitive = true;

    ArgumentParser.prototype.alias = function(arg) {
      return arg;
    };

    ArgumentParser.prototype.consume = function(args) {
      var a, alist, arglist, cmd, collision, extraopts, kwargs, ns, subp, varargs, _name, _ref, _ref1, _ref2, _ref3;
      arglist = (function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = args.length; _i < _len; _i++) {
          a = args[_i];
          _results.push(this.alias(a));
        }
        return _results;
      }).call(this);
      cmd = null;
      if (this.subparsers != null) {
        _ref = this._extract_subparser_cmd(arglist), subp = _ref[0], cmd = _ref[1];
        if (!(subp != null) && (cmd != null)) {
          return [cmd, this.missing(cmd)];
        } else if (subp != null) {
          '';

        }
      }
      if ((this.argspec != null) && !_.isEmpty(this.argspec.varkw)) {
        _ref1 = this._extract_kwargs(arglist), arglist = _ref1[0], kwargs = _ref1[1];
      } else {
        kwargs = {};
      }
      if ((this.argspec != null) && this.argspec.varargs.length > 0) {
        _ref2 = this.parse_known_args(arglist), ns = _ref2[0], extraopts = _ref2[1];
      } else {
        _ref3 = [this.parse_args(arglist), []], ns = _ref3[0], extraopts = _ref3[1];
      }
      args = (function() {
        var _i, _len, _ref4, _results;
        _ref4 = this.argspec.args;
        _results = [];
        for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
          a = _ref4[_i];
          _results.push(ns[a]);
        }
        return _results;
      }).call(this);
      varargs = typeof ns[_name = this.argspec.varargs] === "function" ? ns[_name]([]) : void 0;
      collision = [];
      if (collision.length > 0) {
        this.error('colliding keyword arguments:' + collision);
      }
      alist = [].concat(args, varargs, extraopts);
      return [cmd, this.func(alist, kwargs)];
    };

    ArgumentParser.prototype._extract_subparse_cmd = function(arglist) {
      var arg, cmd, i, name_parser_map, optprefix, _i, _j, _len, _ref, _ref1, _ref2, _results;
      optprefix = this.prefix_chars[0];
      name_parser_map = this.subparsesrs._name_parser_map;
      _ref1 = _.zip((function() {
        _results = [];
        for (var _j = 0, _ref = arglist.length; 0 <= _ref ? _j < _ref : _j > _ref; 0 <= _ref ? _j++ : _j--){ _results.push(_j); }
        return _results;
      }).apply(this), arglist);
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        _ref2 = _ref1[_i], i = _ref2[0], arg = _ref2[1];
        if (arg[0] !== optprefix) {
          cmd = _match_cmd(arg, name_parser_map, this.case_sensitive);
          delete arglist[i];
          return [name_parser_map[cmd], cmd || arg];
        }
      }
      return [null, null];
    };

    ArgumentParser.prototype.addsubcommands = function(commands, obj, title, cmdprefix) {
      var add_help, cmd, func, options, prefixlen, subparser, _i, _len, _ref, _ref1, _results;
      if (title == null) {
        title = null;
      }
      if (cmdprefix == null) {
        cmdprefix = '';
      }
      if (!(this.subparsers != null)) {
        this.subparsers = this.add_subparsers({
          title: title
        });
      } else if (title != null) {
        this.add_argument_group({
          title: title
        });
      }
      prefixlen = ((_ref = obj.cmdprefix) != null ? _ref : '').length;
      add_help = (_ref1 = obj.add_help) != null ? _ref1 : true;
      _results = [];
      for (_i = 0, _len = commands.length; _i < _len; _i++) {
        cmd = commands[_i];
        func = obj[cmd.slice(prefixlen)];
        options = {
          add_help: add_help,
          help: 'func.__doc__'
        };
        subparser = this.subparsers.add_parser(cmd, options);
        _results.push(subparser.populate_from(func));
      }
      return _results;
    };

    ArgumentParser.prototype._set_func_argspec = function(obj) {
      this.func = obj;
      this.argspec = getargspec(obj);
      return _parser_registry[obj] = this;
    };

    ArgumentParser.prototype.populate_from = function(func) {
      var a, alldefaults, ann, defaultValue, defaults, dflt, f, i, metavar, n_args, n_defaults, name, prefix, shortlong, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
      this._set_func_argspec(func);
      f = this.argspec;
      defaults = (_ref = f.defaults) != null ? _ref : [];
      n_args = f.args.length;
      n_defaults = defaults.length;
      alldefaults = ((function() {
        var _i, _ref1, _results;
        _results = [];
        for (i = _i = 0, _ref1 = n_args - n_defaults; 0 <= _ref1 ? _i < _ref1 : _i > _ref1; i = 0 <= _ref1 ? ++_i : --_i) {
          _results.push(null);
        }
        return _results;
      })()).concat(defaults);
      prefix = this.prefix = ((_ref1 = func.prefix_chars) != null ? _ref1 : '-')[0];
      _ref2 = _.zip(f.args, alldefaults);
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        _ref3 = _ref2[_i], name = _ref3[0], defaultValue = _ref3[1];
        ann = (_ref4 = f.annotations[name]) != null ? _ref4 : [];
        a = annotation_from(ann);
        metavar = a.metavar;
        if (!(defaultValue != null)) {
          dflt = null;
        } else {
          dflt = defaultValue;
          if (!(a.help != null)) {
            a.help = "[" + dflt + "]";
          }
        }
        if ((_ref5 = a.kind) === 'option' || _ref5 === 'flag') {
          if (a.abbrev != null) {
            shortlong = [prefix + a.abbrev, prefix + prefix + name.replace('_', '-')];
          } else {
            shortlong = [prefix + name.replace('_', '-')];
          }
        } else if (!(defaultValue != null)) {
          this.add_argument(name, {
            help: a.help,
            type: a.type,
            choices: a.choices,
            metaver: metavar
          });
        } else {
          this.add_argument(name, {
            nargs: '?',
            help: a.help,
            defaultValue: dflt,
            type: a.type,
            choices: a.choices,
            metavar: metavar
          });
        }
        if (a.kind === 'option') {
          if (defaultValue != null) {
            metavar = metavar != null ? metavar : "" + metavar;
          }
          this.add_argument.apply(this, __slice.call(shortlong).concat([{
            help: a.help,
            defaultValue: dflt,
            type: a.type,
            choices: a.choices,
            metavar: metavar
          }]));
        } else if (a.king === 'flag') {
          if ((defaultValue != null) && defaultValue !== false) {
            throw new TypeError("Flat " + name + " wants default false, got " + defaultValue);
          }
          this.add_argument.apply(this, __slice.call(shortlong).concat([{
            action: 'storeTrue',
            help: a.help
          }]));
        }
      }
      if (f.varargs.length > 0) {
        a = annotation_from((_ref6 = f.annotations[f.varags]) != null ? _ref6 : []);
        return this.add_argument(f.varkw, {
          nargs: '*',
          help: a.help,
          defaultValue: {},
          type: a.type,
          metavar: a.metaver
        });
      }
    };

    ArgumentParser.prototype.missing = function(name) {
      var miss, _ref;
      miss = (_ref = this.obj['__missing__']) != null ? _ref : function(name) {
        return this.error("No command " + name);
      };
      return miss(name);
    };

    ArgumentParser.prototype.print_actions = function() {
      var a, _i, _len, _ref, _results;
      console.log(this);
      _ref = this._actions;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        a = _ref[_i];
        _results.push(console.log(a));
      }
      return _results;
    };

    return ArgumentParser;

  })(argparse.ArgumentParser);

  iterable = function(obj) {
    return (obj.__iter__ != null) && !_.isString(obj);
  };

  call = function(obj, arglist, eager) {
    var cmd, result, _ref;
    if (arglist == null) {
      arglist = process.argv.slice(2);
    }
    if (eager == null) {
      eager = true;
    }
    _ref = parser_from(obj).consume(arglist), cmd = _ref[0], result = _ref[1];
    return result;
  };

  exports.call = call;

  /*
  # Convert anything iterable into a real, live array.
  _.toArray = (iterable) ->
    return []                   if (!iterable)
    return iterable.toArray()   if (iterable.toArray)
    return iterable             if (_.isArray(iterable))
    return slice.call(iterable) if (_.isArguments(iterable))
    _.values(iterable)
  */


  if (!(module.parent != null)) {
    main = function(dsn) {
      console.log(dsn);
      return 'Done';
    };
    console.log(call(main, ['DSN obj']));
  }

  /*
  in py
  def foo3(x,y=3,a=None,*z,**w):
      print x,y,z,w
      return 'done'
     ....: 
  
  vars(plac.getargspec(foo3)) 
  {'annotations': {},
   'args': ['x', 'y', 'a'],
   'defaults': (3, None),
   'varargs': 'z',
   'varkw': 'w'}
  
  in js, there are only args
  coffee adds defaults (of sorts)
  and something like *z, array turned into string of args 
  but those aren't directly visible in underlying js
  
  square = function(x) {
    return x * x;
  };
  square.toString()
  square.length   # of named arguments
  
  function square(x) {...} will have square.name = 'square'
  bar = function foo(x)
  */


}).call(this);
