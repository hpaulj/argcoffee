...
var Annotation, ArgumentParser, DEBUG, GetFullArgspec, PARSER_CFG, annotations, argparse, assert, call, getargspec, is_annotation, iterable, parse_function, parser_from, pconf, source, sources, _, _extract_kwargs, _i, _len, _match_cmd, _parser_registry,
  __slice = [].slice,
...
parse_function = function(fn) {
  var FN_ARG, FN_ARGS, FN_ARG_SPLIT, STRIP_COMMENTS, arg_decl, args, cmts, doc, fn_text, name, pushfn, r;
  ...
  pushfn = function(all, name) {
    return args.push(name);
  };
  _.each(r, function(arg) {
    return arg.replace(FN_ARG, pushfn);
  });
  name = name || null;
  doc = doc || null;
...
GetFullArgspec = (function() {
  ...
  function GetFullArgspec(fn) {
    var first, last, _j, _k, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
    _ref = parse_function(fn), this.args = _ref[0], this.name = _ref[1], this.doc = _ref[2];
    this.varargs = null;
    this.varkw = null;
    _ref1 = this.args, first = 2 <= _ref1.length ? __slice.call(_ref1, 0, _j = _ref1.length - 1) : (_j = 0, []), last = _ref1[_j++];
    if (last === 'kwarg' || last === 'kwargs' || last === '__kw' || last === 'varkw') {
      _ref2 = [first, last], this.args = _ref2[0], this.varkw = _ref2[1];
    }
    _ref3 = this.args, first = 2 <= _ref3.length ? __slice.call(_ref3, 0, _k = _ref3.length - 1) : (_k = 0, []), last = _ref3[_k++];
    if (last === 'vargs' || last === '_arg') {
      _ref4 = [first, last], this.args = _ref4[0], this.varargs = _ref4[1];
    }
    this.defaults = (_ref5 = fn.defaults) != null ? _ref5 : [];
    this.annotations = (_ref6 = fn.__annotations__) != null ? _ref6 : {};
  }
...
GetFullArgspec = (function() {
  ...
  function GetFullArgspec(fn) {
    ...
    this.varargs = null;
    this.varkw = null;
    first = _.initial(this.args);
    last = _.last(this.args);
    ...
    this.defaults = fn.defaults || [];
    this.annotations = fn.__annotations__ || {};
  }
...
annotations = function(ann) {
  ...
  annotate = function(f) {
    ...
    for (argname in ann) {
      if (__indexOf.call(args, argname) < 0) {
...
_extract_kwargs = function(args) {
  var arg, arglist, kwargs, m, name, _j, _len1;
  arglist = [];
  kwargs = {};
...
ArgumentParser = (function(_super) {
  ...
  ArgumentParser.prototype.consume = function(args) {
    ...
    if ((this.argspec != null) && (this.argspec.varargs != null)) {
      ...
      _ref3 = [this.parseArgs(arglist), []], ns = _ref3[0], extraopts = _ref3[1];
    }
    DEBUG('ns', ns);
  ...
  ArgumentParser.prototype._extract_subparser_cmd = function(arglist) {
    var arg, cmd, i, name_parser_map, optprefix, _j, _len1, _ref, _ref1;
    optprefix = ((_ref = this.prefix_chars) != null ? _ref : this.prefixChars)[0];
    name_parser_map = (_ref1 = this.subparsers._name_parser_map) != null ? _ref1 : this.subparsers._nameParserMap;
    for (i = _j = 0, _len1 = arglist.length; _j < _len1; i = ++_j) {
      arg = arglist[i];
      if (!(arg[0] !== optprefix)) {
        continue;
      }
  ...
  ArgumentParser.prototype.addsubcommands = function(commands, obj, title, cmdprefix) {
    var add_help, cmd, func, options, prefixlen, subparser, _j, _len1, _ref, _ref1, _results;
    if (title == null) {
      title = null;
    }
    if (cmdprefix == null) {
  ...
  ArgumentParser.prototype.populate_from = function(func) {
    var a, alldefaults, ann, defaultValue, defaults, dflt, f, metavar, n_args, n_defaults, name, nargs, prefix, shortlong, _j, _len1, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
    this._set_func_argspec(func);
    f = this.argspec;
    defaults = (_ref = f.defaults) != null ? _ref : [];
    n_args = f.args.length;
    n_defaults = defaults.length;
    alldefaults = _.times(n_args - n_defaults, function() {
      return null;
    }).concat(defaults);
    ...
    for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
      _ref3 = _ref2[_j], name = _ref3[0], defaultValue = _ref3[1];
...
if (module.parent == null) {
  (function() {
    var afunc, d, main, p4, parser, parser_from1;
